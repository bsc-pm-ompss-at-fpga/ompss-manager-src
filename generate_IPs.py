...
class ArgParser:
    def __init__(self):
        ...
        self.parser.add_argument('--disable_tm', help='disables generation of Task Manager IP', action='store_true', default=False)
        self.parser.add_argument('--disable_extended_tm', help='disables generation of Extended Task Manager IP', action='store_true', default=False)
...
def generate_IP(extended=False):
    msg.info('Generating ' + ('Extended ' if extended else '') + 'Task Manager IP')

    if extended:
        dst_path = './ompss_manager_IP/Vivado_HLS/ext_tm/command_etm'
        script = 'extended_task_manager_ip_packager.tcl'
    else:
        dst_path = './ompss_manager_IP/Vivado_HLS/command_tm'
        script = 'task_manager_ip_packager.tcl'
    ...
    if retval:
        ...
        msg.error('Generation of ' + ('Extended ' if extended else '') + 'Task Manager IP failed')
    else:
        msg.success('Finished generation of ' + ('Extended ' if extended else '') + 'Task Manager IP')
...
def synthesize_hls(file_, extended=False):
    ...
    #FIXME
    ##Put the accelerators configuration in the Scheduler TM now
    #if 'Scheduler_Task_Manager.cpp' in file_:
    #    for line in fileinput.input(project_Vivado_HLS_path + '/' + acc_name + '/' + acc_file, inplace=True):
    #        line = line.replace('//PLACEHOLDER_FOR_TASK_TYPES_INFO', sched_tm_info)
